...
class CrossAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        self_output, self_att = self.self(input_tensor, attention_mask)
        attention_output = self.output(self_output, input_tensor)
        return attention_output, self_att
...
class CrossLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        attention_output, att_weight = self.attention(hidden_states, attention_mask)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        return layer_output, att_weight
...
class CrossEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        for layer_module in self.layer:
            hidden_states, att_weights = layer_module(hidden_states, attention_mask)
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers, att_weights
...
class CrossModel(PreTrainedModel):
    ...
    def forward(self, concat_input, concat_type=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        encoded_layers, att_weight = self.encoder(embedding_output,
        ...
        return encoded_layers, pooled_output, att_weight
